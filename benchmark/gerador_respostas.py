...
# Para rodar as 60 em PT e 60 em EN, mantenha a lista com as duas tuplas:
...
class BenchmarkRunner:
    ...
    def _build_streaming_kwargs(self, prompt: str) -> Dict[str, Any]:
        kwargs = {
            ...
            "seed": SEED_GLOBAL, # Puxando do Painel de Controle
            "stream": True,
        ...
        if not any(modelo in model_lower for modelo in modelos_sem_temperature):
            kwargs["temperature"] = TEMP_GLOBAL
...
class TutorInteligenteRunner(BenchmarkRunner):
    ...
    def generate_response(self, prompt: str) -> Dict[str, Any]:
        ...
        # Puxando o SEED e TEMP diretamente do Painel de Controle Global
        payload = {
            "query": prompt,
            "model": self.model_name,
            "seed": SEED_GLOBAL
        ...
        # Trava de segurança: remove a temperature se for gpt-5
        modelos_sem_temperature = ["o1", "o3", "gpt-5"]
...
def process_dataset():
    ...
    # Adicionando um sufixo dinâmico para sabermos se foi o dataset completo ou teste
